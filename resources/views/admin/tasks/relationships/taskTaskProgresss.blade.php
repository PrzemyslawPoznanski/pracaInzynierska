<div class="m-3">
    @can('task_progress_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.task-progresss.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.taskProgress.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.taskProgress.title_singular') }} {{ trans('global.list') }}
        </div>

        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-taskTaskProgresss">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        @foreach($taskProgresss as $key => $taskProgress)
                            <tr data-entry-id="{{ $taskProgress->id }}">
                                <td>
...
@can('task_progress_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.task-progresss.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-taskTaskProgresss:not(.ajaxTable)').DataTable({ buttons: dtButtons })
